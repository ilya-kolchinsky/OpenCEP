...
class AdaptiveBushyTreeBuilder(EvaluationMechanismBuilder):
    """
...
class AdaptiveDynamicProgrammingBushyTreeBuilder(AdaptiveBushyTreeBuilder):
    """
...
class AdaptiveZStreamTreeBuilder(AdaptiveBushyTreeBuilder):
    """
...
class AdaptiveZStreamOrdTreeBuilder(AdaptiveZStreamTreeBuilder):
    """
    Creates a left-deep tree using ZStream algorithm with the leaf order obtained using an order-based greedy algorithm.
    """
    @staticmethod
    def _get_initial_order(selectivity_matrix: List[List[float]], arrival_rates: List[int]):
        return AdaptiveGreedyLeftDeepTreeBuilder.calculate_greedy_order(selectivity_matrix, arrival_rates)
