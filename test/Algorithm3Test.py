...
"""
def samePatternSharingRootAlgorithm3(createTestFile = False):
    hierarchyPattern = Pattern(
    ...
    attribute_dict = {}
    types = hierarchyPattern.get_all_event_types_with_duplicates()
    types.extend(hierarchyPattern2.get_all_event_types_with_duplicates())
    types.extend(hierarchyPattern3.get_all_event_types_with_duplicates())
    for type in types:
        if type in attribute_dict.keys():
            attribute_dict[type].append(random.choice(attributes))
        else:
...
"""
def multipleParentsForInternalNodeAlgorithm3(createTestFile = False):
    pattern1 = Pattern(
        SeqOperator(PrimitiveEventStructure("AAPL", "a"),
    ...
    for type in types:
        if type in attribute_dict.keys():
            ...
            attribute_dict[type] = [random.choice(attributes)]
    runMultiTest("multipleParentsForInternalNode", [pattern1, pattern2, pattern3, pattern4], eval_mechanism_params,
                 parallel_execution_params=ParallelExecutionParameters(ParallelExecutionModes.DATA_PARALLELISM,
                                                                       ParallelExecutionPlatforms.THREADING),
                 data_parallel_params=DataParallelExecutionParameters(DataParallelExecutionModes.ALGORITHM3,
                                                                      num_threads=100,
