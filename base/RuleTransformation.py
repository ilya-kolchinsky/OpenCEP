...
class RuleTransformation:
    def pattern_transformation(pattern : Pattern):
        ...
        rules_priority_list = RuleTransformation.get_priority_list()
        for rule_name in rules_priority_list:
            if rule_name == "AND_AND":
                transformed_pattern_structures_list = RuleTransformation.transform_and_and(transformed_pattern_structures_list)
            elif rule_name == "SEQ_OR":
                transformed_pattern_structures_list = RuleTransformation.transform_seq_or(transformed_pattern_structures_list)
        ...
        if EXPAND_PATTERN_AND_AND:
            for i in range(len(pattern_structures_list)):
                ...
                if old_pattern_structure.get_top_operator() == AndOperator:
                    ...
                    tmp_pattern_structure.add_args(RuleTransformation.expand_and_operator(old_pattern_structure))
        ...
        if EXPAND_PATTERN_SEQ_OR:
            for i in range(len(pattern_structures_list)):
                ...
                if old_pattern_structure.get_top_operator() == SeqOperator:
                    new_args_list = RuleTransformation.expand_or_operator(old_pattern_structure)
    ...
    def expand_and_operator(pattern_structure : PatternStructure):
        ...
        for arg in pattern_structure.get_args():
            if type(arg) == AndOperator:
                new_args.extend(RuleTransformation.expand_and_operator(arg))
            else:
                new_args.append(arg)
        return new_args
