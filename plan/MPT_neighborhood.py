...
from SimulatedAnnealing import tree_plan_visualize_annealing, TreeBasedEvaluationMechanism, visualize_annealing_timed
...
class algoA(TreePlanBuilder):
    ...
    def Nvertex_neighborhood(pattern_to_tree_plan_map: Dict[Pattern, TreePlan] or TreePlan,
        ...
        for j in range(len(patterns_list)):
            ...
            is_j_contain_sub_pattern = len(list(filter(lambda pattern: pattern.is_equivalent(sub_pattern), sub_patterns_i_j))) > 0
...
def annealing_advanced_test_timed():
    pattern1 = Pattern(
