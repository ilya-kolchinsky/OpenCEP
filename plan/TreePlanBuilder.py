...
class TreePlanBuilder(ABC):
    ...
    def share_leaves(pattern_to_tree_plan_map: Dict[Pattern, TreePlan] or TreePlan):
        ...
        for i, pattern in enumerate(pattern_to_tree_plan_map):
            ...
            leaves_dict[pattern] = TreePlanBuilder.tree_get_leaves(pattern.positive_structure, tree_plan.root,
                                                                   TreePlanBuilder.__get_operator_arg_list(
    ...
    def tree_get_leaves(root_operator: PatternStructure, tree_plan: TreePlanNode,
        ...
        if type(tree_plan) == TreePlanLeafNode:
            ...
            return leaves_nodes
        leaves_nodes = TreePlanBuilder.tree_get_leaves(root_operator, tree_plan.left_child, args, sliding_window, None,
                                                       consumption_policy, leaves_nodes)
        leaves_nodes.update(TreePlanBuilder.tree_get_leaves(root_operator, tree_plan.right_child, args,
                                                            sliding_window, None, consumption_policy, leaves_nodes))
        return leaves_nodes
